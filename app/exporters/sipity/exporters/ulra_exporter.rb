module Sipity
  # :nodoc:
  module Exporters
    # Export a ULRA work from Sipity to Curate, create ROF file and download
    # attachments and bundle them into a directory
    class UlraExporter
      ...
      def export_to_json
        json_array = []
        json_array << Mappers::UlraMapper.call(work, attribute_map: ULRA_ATTRIBUTES, mount_data_path: MNT_DATA_PATH)
        # build attachment json
        attachments.each do |file|
          json_array << Mappers::GenericFileMapper.call(file, attribute_map: ULRA_ATTRIBUTES, mount_data_path: MNT_DATA_PATH)
        end
        json_array
      end
