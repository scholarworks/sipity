    ...
    RSpec.describe DoisController, type: :controller do
      ...
      context 'GET #show' do
        ...
        context 'when :doi_not_assigned' do
          ...
          it 'will render the show page' do
            ...
            expect(assigns(:model).sip).to be_decorated
        ...
        context 'when :doi_already_assigned' do
          ...
          it 'will redirect to the sip page' do
            ...
            expect(response).to redirect_to sip_path(sip.to_param)
          end
        end

        context 'when :doi_request_is_pending' do
          let(:doi_request) { double }
          let(:yields) { [sip, doi_request] }
      ...
      context 'POST #assign_a_doi' do
        ...
        let(:runner) do
          Hesburgh::Lib::MockRunner.new(
            ...
            run_with: { sip_id: sip.to_param, identifier: identifier }
          )
        end

        context 'when :success' do
          let(:yields) { [sip, identifier] }
          let(:callback_name) { :success }
          let(:identifier) { 'doi:abc' }
          it 'will redirect to the sip page' do
            post 'assign_a_doi', sip_id: sip.to_param, doi: { identifier: identifier }
            expect(flash[:notice]).to_not be_empty
            expect(response).to redirect_to(sip_path(sip.to_param))
          end
        end
      ...
      context 'POST #request_a_doi' do
        ...
        context 'when :success' do
          ...
          let(:identifier) { 'doi:abc' }
          it 'will redirect to the sip page' do
            post 'request_a_doi', sip_id: sip.to_param, doi: attributes
            expect(flash[:notice]).to_not be_empty
            expect(response).to redirect_to(sip_path(sip.to_param))
          end
        end
