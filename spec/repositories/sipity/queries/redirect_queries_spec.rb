    ...
    RSpec.describe RedirectQueries, type: :isolated_repository_module do
      ...
      context '#active_redirect_for' do
        it 'will find the most relevant redirect' do
          ...
          expect(test_repository.active_redirect_for(work: work)).to eq(current)
          expect(test_repository.active_redirect_for(work: work, as_of: 5.days.ago.to_date)).to be_nil
        end
      end
    end
  end
