  ...
  module Services
    RSpec.describe LdapClient do
      ...
      context "#netid_valid?" do
        subject { LdapClient.new(netid, ldap_options) }
        let(:netid) { 'a possible netid' }
        context 'with a valid netid' do
          it "will return true for a valid netid" do
            expect(subject).to receive(:ldap_lookup).and_return(true)
            expect(subject.netid_valid?).to eq(true)
          end
        end
        context "with invalid net id" do
          it "should return false" do
            expect(subject).to receive(:ldap_lookup).and_return(nil)
            expect(subject.netid_valid?).to eq(false)
          end
        end
      end
    end
