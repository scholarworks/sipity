...
RSpec.describe 'PowerConverter' do
  context ':rof_hash' do
    it "will convert a Sipity::Models::Attachment" do
      ...
      expect(Sipity::Conversions::ToRofHash::AttachmentConverter).to receive(:call).with(object).and_return(:converted)
      PowerConverter.convert(object, to: :rof_hash)
    end
    it "will convert a Sipity::Models::Work" do
      object = Sipity::Models::Work.new
      expect(Sipity::Conversions::ToRofHash::WorkConverter).to receive(:call).with(object).and_return(:converted)
      PowerConverter.convert(object, to: :rof_hash)
    end
